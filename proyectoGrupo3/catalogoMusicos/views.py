...
def registro(request):
    if request.method == "POST":
        ...
        if form_registro.is_valid():
            ...
            nueva_cuenta.set_password(
                form_registro.cleaned_data["password"]
            )  # Encripta la contraseña.
            nueva_cuenta.save()
            return render(
                request,
                "catalogoMusicos/registro_completado.html",
                {"nueva_cuenta": nueva_cuenta},
            )
    else:
        form_registro = FormularioRegistro()
    return render(
        request, "catalogoMusicos/registro.html", {"form_registro": form_registro}
    )
...
def registro_musico(request):
    ...
    return render(
        request,
        "catalogoMusicos/registro_musico.html",
...
def listar_anuncios(request):
    ...
    return render(
        request, "catalogoMusicos/anuncios/lista.html", {"anuncios": anuncios}
    )
...
def listar_reportes(request):
    ...
    return render(
        request, "catalogoMusicos/reportes/lista.html", {"reportes": reportes}
    )


def listar_eventos(request):
    eventos = Evento.objects.all()
    return render(request, "catalogoMusicos/eventos/lista.html", {"eventos": eventos})
...
def crear_musico(request):
    if request.method == "POST":
        form = FormularioRegistroMusico(request.POST)
        if form.is_valid():
            musico = form.save(commit=False)
            musico.set_password(form.cleaned_data["password"])
            musico.save()
            return redirect("catalogoMusicos:detalle_musico", id=musico.id)
    else:
        form = FormularioRegistroMusico()

    return render(request, "catalogoMusicos/musicos/crear.html", {"form": form})


@login_required
def editar_musico(request, id):
    musico = get_object_or_404(Musico, id=id)
    # Solo el propio músico puede editar su perfil (comparar pks por seguridad)
    if request.user.pk != musico.pk:
        raise PermissionDenied("No tienes permiso para editar este músico.")
...
def eliminar_musico(request, id):
    musico = get_object_or_404(Musico, id=id)
    # Solo el propio músico puede eliminar su perfil (comparar pks por seguridad)
    if request.user.pk != musico.pk:
        raise PermissionDenied("No tienes permiso para eliminar este músico.")
...
def detalle_anuncio(request, id):
    ...
    return render(
        request, "catalogoMusicos/anuncios/detalle.html", {"anuncio": anuncio}
    )
...
def detalle_reporte(request, id):
    ...
    return render(
        request, "catalogoMusicos/reportes/detalle.html", {"reporte": reporte}
    )
...
def detalle_evento(request, id):
    ...
    return render(request, "catalogoMusicos/eventos/detalle.html", context)
...
def crear_evento(request):
    ...
    return render(
        request,
        "catalogoMusicos/eventos/crear.html",
...
def crear_anuncio(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            form.save_m2m()
            messages.success(
                request, f'Anuncio "{anuncio.titulo}" creado exitosamente!'
            )
    ...
    return render(
        request,
        "catalogoMusicos/anuncios/crear.html",
        {
            "form": form,
        },
    )
...
def crear_banda(request):
    ...
    return render(
        request, "catalogoMusicos/bandas/crear.html", {"form": form, "banda": banda}
    )
...
def editar_reporte(request, id):
    ...
    return render(
        request,
        "catalogoMusicos/reportes/editar.html",
...
def editar_evento(request, id):
    ...
    return render(
        request,
        "catalogoMusicos/eventos/editar.html",
        {
            "form": form,
            "evento": evento,
            "google_maps_api_key": settings.GOOGLE_MAPS_API_KEY,
...
def editar_anuncio(request, id):
    ...
    return render(
        request,
        "catalogoMusicos/anuncios/editar.html",
        {"form": form, "anuncio": anuncio},
    )
...
def editar_banda(request, id):
    ...
    if banda.usuario != request.user:
        raise PermissionDenied(
            "No tienes permiso para editar esta banda porque no eres el creador."
        )
    ...
    return render(
        request, "catalogoMusicos/bandas/editar.html", {"form": form, "banda": banda}
    )
...
def eliminar_anuncio(request, id):
    ...
    return render(
        request, "catalogoMusicos/anuncios/eliminar.html", {"anuncio": anuncio}
    )
...
def eliminar_banda(request, id):
    ...
    if banda.usuario != request.user:
        raise PermissionDenied(
            "No tienes permiso para eliminar esta banda porque no eres el creador."
        )
